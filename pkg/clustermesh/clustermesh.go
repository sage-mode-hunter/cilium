...
type Configuration struct {
	...
	common.Config
	cmtypes.ClusterMeshServiceModeV2Config
	wait.TimeoutConfig
...
func (cm *ClusterMesh) NewRemoteCluster(name string, status common.StatusFunc) common.RemoteCluster {
	rc := &remoteCluster{
		...
		clusterMeshServiceV2:     cm.conf.ClusterMeshServiceV2,
...
func registerLoadBalancerInitialized(jobs job.Group, cm *ClusterMesh, cfg cmtypes.ClusterMeshServiceModeV2Config, w *writer.Writer) {
	if cm == nil {
		return
	}
	...
	jobs.Add(
		job.OneShot(
			...
			func(ctx context.Context, health cell.Health) error {
				var err error
				if cfg.ClusterMeshServiceV2.WatchLegacyServices() {
					err = cm.ServicesSynced(ctx)
				} else if cfg.ClusterMeshServiceV2.WatchEndpointSlices() {
